...
router.post("/",async (req,res)=>{
    ...
    const verify_role_payload = {
        task:'products',
        sub_task: 'un-feature_product',
        role: payload.auth_role
    }
    const verified_result = await Role_Verifier(verify_role_payload);
    //console.log(verified_result)
    if (!verified_result){
        return res.status(401).send("You are not authorized to un-feature this product, kindly contact the administrator or support for any issues");
    }else{
        ...
        try{
            ...
            return res.status(500).send("Could not un-feature this product, try again in a few minutes");
        }
    }
})
